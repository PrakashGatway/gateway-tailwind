...
function About({ aboutPage: data }: any) {
  const [heroData, setHeroData] = useState(null);
  const [whoAreWeData, setWhoAreWeData] = useState([]);
  ...
  useEffect(() => {
    if (data) {
      // Store pageContent for image URLs
      if (data.pageContent) {
      ...
      if (data.sections) {
        data.sections.forEach((section) => {
          switch (section.type) {
            case 'hero':
              setHeroData(section.content);
              break;
            case 'WhoAreWe':
              setWhoAreWeData(section.content.items || []);
              break;
            case 'data':
              setCounterData(section.content.items || []);
              break;
            case 'resources':
              setTeamData(section.content.items || []);
              break;
            case 'counselling':
              setCounsellingData(section.content);
              break;
          }
        });
      }
      ...
      <div>
        {heroData && (
          <HeroSection
            ...
            image={pageContent?.heroImage ? getImageUrl(pageContent.heroImage) : `/img/about-us-banner-img.svg`}
          />
        )}
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                    <div className="w-full md:w-2/5">
                      <div className="flex justify-center">
                        <Image
                          src={pageContent?.file ? getImageUrl(pageContent.file) : "/img/about-us-img-1.svg"}
                    ...
                    <div className="w-full md:w-3/5 sm:px-[3rem]">
                      <div className="md:pl-6 lg:pl-8">
                        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-4 md:mb-6 pb-3 md:pb-4">
                          {whoAreWeData[0]?.sectionTitle || 'Who Are We?'}
                        </h2>
                        <div 
                          className="text-gray-700 text-base leading-6 sm:leading-7 text-justify"
                          dangerouslySetInnerHTML={{ 
                            __html: whoAreWeData[0]?.description || '' 
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                      <div className="w-full md:w-3/5 order-2 md:order-1">
                        <div className="md:pr-6 lg:pr-8">
                          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-4 md:mb-6 pb-3 md:pb-4">
                            {whoAreWeData[1]?.sectionTitle || 'What Will We Do for You?'}
                          </h2>
                          <div 
                            className="text-gray-700 text-base leading-6 sm:leading-7 text-justify"
                            dangerouslySetInnerHTML={{ 
                              __html: whoAreWeData[1]?.description || '' 
                            }}
                          />
                        </div>
                      </div>
                      <div className="w-full md:w-2/5 order-1 md:order-2">
                        <div className="flex justify-center">
                          <Image
                            src={pageContent?.file1 ? getImageUrl(pageContent.file1) : "/img/about-us-img-2.svg"}
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-[7rem] max-w-6xl relative z-10">
              <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-3">
                {counterData.map((item, index) => (
                  <div key={index} className="py-6 sm:py-8 md:py-[45px] text-center w-full rounded-[16px] bg-white/80 dark:bg-gray-800/80 shadow-[0_4px_4px_0_rgba(0,0,0,0.25)]">
                    <div className="flex items-center justify-center">
                      <h4
                        className="count text-2xl sm:text-3xl md:text-4xl font-bold text-red-600 dark:text-white mb-2"
                        data-target={parseInt(item.number)}
                      >
              ...
              <div className="pt-6 md:pt-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 justify-items-center">
                  {teamData.map((member, index) => (
                    <div key={index} className="bg-[#fcedf0] rounded-xl shadow-lg overflow-hidden border border-gray-200 w-full max-w-[550px]">
                      <div className="bg-red-600 px-4 sm:px-6 py-3 sm:py-4 text-center">
                        <h4 className="text-lg sm:text-xl font-bold text-white">{member.title}</h4>
                      </div>
                ...
                <div className="px-4 sm:px-6 lg:px-8">
                  <div className="flex flex-col lg:flex-row items-center gap-6 sm:gap-8">
                    <div className="w-full lg:w-[48%]">
                      <div className="text-center lg:text-left pl-[17px]">
                        <h2 className="text-xl sm:text-2xl lg:text-3xl xl:text-[36px] font-bold mb-4 text-[#D71635] lg:leading-[37px]">
                          {counsellingData.title || 'Avail A Complementary Counselling Session'}
                        </h2>
                        <p className="text-base sm:text-lg lg:text-[18px] mb-4 sm:mb-6 text-[#666276]">
                          {counsellingData.description || 'Join thousand of instructors and earn money hassle free!'}
                        </p>
                        <a
                    ...
                    <div className="w-full lg:w-[38%]">
                      <div className="flex justify-center">
                        <img
                          src={pageContent?.image ? getImageUrl(pageContent.image) : "img/counselling-session.svg"}
...
// function About({ aboutPage: data, teamMembers: member }: any) {
...
//                       Who Are We?
//                     </h2>
//                     <p className="text-gray-700 text-base  leading-6 sm:leading-7 text-justify">
